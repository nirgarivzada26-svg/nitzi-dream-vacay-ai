...
export const Route = createFileRoute("/_authenticated/account")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "הזמנות, חופשות שמורות, מועדפים, היסטוריית חיפוש והגדרות התראות.",
      },
...
function AccountPage() {
  ...
  return (
    <div
      dir="rtl"
      className="min-h-screen bg-gradient-to-b from-sand/50 via-background to-background pb-16"
    >
...
function BookingsTab() {
  ...
  if (!q.data?.length)
    return (
      <EmptyState
        title="עוד אין לך הזמנות"
        hint="ההזמנות שאתה מבצע דרך NITZI יופיעו כאן עם מספר אישור ופרטים מלאים."
        cta={
          <Link
            to="/"
      ...
      {q.data.map((b) => (
        <div key={b.id} className="rounded-3xl border border-border bg-card p-6 shadow-soft">
          ...
          <div className="mt-1 flex flex-wrap gap-2 text-[11px] text-muted-foreground">
            <span>
              <Calendar className="mr-0.5 inline h-3 w-3" /> {b.start_date} → {b.end_date}
            </span>
            <span>
              · {b.nights} לילות · {b.people} נוסעים
            </span>
          </div>
          <div className="mt-3 flex items-end justify-between">
            <div>
              ...
              <div className="text-2xl font-black text-foreground">
                ₪{Math.round(Number(b.total_price)).toLocaleString()}
              </div>
            </div>
            <Link
              to="/booking/$id"
...
function TripsTab() {
  ...
  if (!q.data?.length)
    return (
      <EmptyState
        title="עדיין לא שמרת אף חופשה"
        hint="בעמוד התוצאות תוכל לשמור מסלול שלם וחזרת אליו מכאן."
        cta={
          <Link
            to="/ai"
            className="rounded-2xl bg-gradient-sunset px-4 py-2 text-sm font-black text-white shadow-glow"
  ...
  return (
    <ul className="grid gap-3">
      {q.data.map((t) => (
        <li
          key={t.id}
          className="flex items-center justify-between rounded-3xl border border-border bg-card p-4 shadow-soft"
        >
          <div>
            <h3 className="text-sm font-black text-foreground">{t.title}</h3>
            <div className="mt-1 flex items-center gap-1.5 text-[11px] text-muted-foreground">
              <MapPin className="h-3 w-3" /> {t.destination_name}
            </div>
          </div>
          <button
            onClick={async () => {
              await deleteSavedTrip(t.id);
              qc.invalidateQueries({ queryKey: ["saved_trips"] });
            }}
...
function FavoritesTab() {
  ...
  if (!q.data?.length)
    return (
      <EmptyState
        title="אין לך עדיין מועדפים"
        hint="בכל דיל לחץ על הלב כדי לשמור לרשימה שלך."
        cta={
          <Link
            to="/"
            className="rounded-2xl bg-gradient-sunset px-4 py-2 text-sm font-black text-white shadow-glow"
        ...
        return (
          ...
          >
            <div className="relative h-48 w-full">
              ...
              <button
                onClick={async () => {
                  await removeFavorite(f.deal_id);
            ...
            <div className="p-4">
              ...
              <div className="mt-2 flex items-center justify-between">
                <div className="text-sm font-black text-foreground">
                  ₪{snap.price.perPerson.toLocaleString()}
                </div>
                <Link
                  to="/deal/$id"
                  params={{ id: f.deal_id }}
...
function HistoryTab() {
  ...
  if (q.isLoading) return <SkeletonList />;
  if (!q.data?.length)
    return <EmptyState title="אין היסטוריית חיפוש" hint="חיפושים שתבצע דרך השאלון יופיעו כאן." />;
  return (
    <div>
      ...
      <ul className="grid gap-2">
        {q.data.map((h) => (
          ...
          >
            <div className="flex items-center justify-between">
              <span className="font-bold text-foreground">
                {h.destination_name ?? "חיפוש חופשי"}
              </span>
  ...
  return (
    <div className="rounded-3xl border border-border bg-card p-6 shadow-soft">
      ...
      <p className="mt-1 text-xs text-muted-foreground">
        בחר אילו התראות תרצה לקבל. אפשר לבטל בכל רגע.
      </p>
      <div className="mt-4 space-y-3">
        <Toggle
          label="דילים חדשים והתראות מיוחדות"
          v={local.deals}
          onChange={(v) => set({ deals: v })}
        />
        <Toggle label="עדכונים באימייל" v={local.email} onChange={(v) => set({ email: v })} />
        <Toggle
          label="הודעות SMS / WhatsApp (בעתיד)"
          v={local.sms}
          onChange={(v) => set({ sms: v })}
          disabledNote="יופעל כשנחבר ספק SMS"
        />
        <Toggle
          label="התראות Push בדפדפן (בעתיד)"
          v={local.push}
          onChange={(v) => set({ push: v })}
          disabledNote="דורש רישום Push"
        />
      </div>
    </div>
  );
...
function Toggle({
  ...
  return (
    <label className="flex items-start justify-between gap-3 rounded-2xl border border-border bg-muted/40 p-3">
      ...
      >
        <span
          className={`absolute top-0.5 h-5 w-5 rounded-full bg-white shadow transition ${v ? "right-0.5" : "right-[calc(100%-1.375rem)]"}`}
        />
      </button>
    </label>
  );
...
function ProfileTab() {
  ...
  useEffect(() => {
    if (q.data) setName(q.data.display_name ?? "");
  }, [q.data]);
