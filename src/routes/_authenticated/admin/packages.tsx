...
function PackagesPage() {
  ...
  const [confirm, setConfirm] = useState<{ action: "delete" | "archive"; slugs: string[] } | null>(
    null,
  );
  ...
  const act = useMutation({
    mutationFn: (v: {
      action: "archive" | "activate" | "delete" | "duplicate" | "feature" | "unfeature";
      slugs: string[];
  ...
  return (
    <div className="space-y-5">
      ...
      <p className="text-xs text-muted-foreground">
        כל חבילה נבנית מיעד פעיל בקטלוג. שדות המלון, המחיר וההרכב מגיעים משכבת הספקים — צפיות,
        הזמנות ושיעור המרה נמדדים מנתוני אמת באתר.
      </p>
            ...
            onClick={() =>
              exportCsv(
                data.map((p) => ({
                  ...
                  מלון: p.hotel ?? "",
                  מחיר: p.price,
                  "מחיר קודם": p.previousPrice ?? "",
                  כוכבים: p.stars ?? "",
                  בסיס: p.board ?? "",
                  סטטוס: p.active ? "פעילה" : "בארכיון",
                  "ציון NITZI": p.nitziScore,
                  צפיות: p.views,
                  הזמנות: p.orders,
        ...
        <DataTable<AdminPackageRow>
          ...
          toolbar={
            selected.length > 0 ? (
              <div className="flex flex-wrap items-center gap-2 rounded-xl bg-muted px-3 py-1.5 text-xs font-bold">
                ...
                <button
                  onClick={() => act.mutate({ action: "activate", slugs: selected })}
                  className="rounded-lg border border-border bg-background px-2 py-1"
                >
                  הפעלה
                </button>
                <button
                  onClick={() => setConfirm({ action: "archive", slugs: selected })}
                  className="rounded-lg border border-border bg-background px-2 py-1"
                >
                  ארכיון
                </button>
                <button
                  onClick={() => act.mutate({ action: "feature", slugs: selected })}
                  className="rounded-lg border border-border bg-background px-2 py-1"
                >
                  הצג בדף הבית
                </button>
                <button
                  onClick={() => act.mutate({ action: "unfeature", slugs: selected })}
                  className="rounded-lg border border-border bg-background px-2 py-1"
                >
                  הסר מדף הבית
                </button>
                <button
                  onClick={() => setConfirm({ action: "delete", slugs: selected })}
          ...
          columns={[
            ...
            { key: "hotel", header: "מלון", render: (r) => r.hotel ?? "—" },
            {
              key: "price",
              header: "מחיר",
              render: (r) => (r.price ? money(r.price) : "—"),
              sortValue: (r) => r.price,
            },
            {
              key: "prev",
            ...
            },
            {
              key: "stars",
              header: "כוכבים",
              render: (r) => (r.stars ? `${r.stars}★` : "—"),
              sortValue: (r) => r.stars ?? 0,
            },
            {
              key: "board",
              header: "בסיס אירוח",
              render: (r) => (r.board ? (boardLabels[r.board as BoardBasis] ?? r.board) : "—"),
            },
            ...
            { key: "beach", header: "מרחק מהחוף", render: (r) => r.beachDistance ?? "—" },
            {
              ...
              render: (r) => (
                <span
                  className={`rounded-full px-2 py-0.5 text-[11px] font-bold ${r.active ? "bg-emerald-100 text-emerald-800" : "bg-muted text-muted-foreground"}`}
                >
                  {r.active ? "פעילה" : "בארכיון"}
                </span>
              ),
            ...
            },
            {
              key: "score",
              header: "ציון NITZI",
              render: (r) => (r.nitziScore ? r.nitziScore : "—"),
              sortValue: (r) => r.nitziScore,
            },
            {
              key: "smart",
              header: "מחיר חכם",
              render: (r) => (
                <span
                  className={`rounded-full px-2 py-0.5 text-[11px] font-bold ${SMART[r.smartPrice].cls}`}
                >
                  {SMART[r.smartPrice].label}
                </span>
              ),
            },
            ...
            },
            {
              ...
              render: (r) => (
                <div className="flex flex-wrap gap-1">
                  ...
                  <button
                    onClick={() =>
                      act.mutate({ action: r.featured ? "unfeature" : "feature", slugs: [r.slug] })
                    }
                    className={`rounded-lg border p-1.5 ${r.featured ? "border-amber-400 text-amber-600" : "border-border"}`}
                    title="מומלץ בדף הבית"
                  >
                    <Star className="h-3.5 w-3.5" />
                  </button>
                  <button
                    onClick={() => act.mutate({ action: "duplicate", slugs: [r.slug] })}
                    className="rounded-lg border border-border p-1.5"
                    title="שכפול"
                  >
                  ...
                  <button
                    onClick={() => setConfirm({ action: "archive", slugs: [r.slug] })}
                    className="rounded-lg border border-border p-1.5"
                    title="ארכיון"
                  >
                  ...
                  <button
                    onClick={() => setConfirm({ action: "delete", slugs: [r.slug] })}
