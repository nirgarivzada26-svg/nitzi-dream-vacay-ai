...
const NAV: { to: string; label: string; icon: React.ComponentType<{ className?: string }>; perm: AdminPermission }[] = [
...
function AdminLayout() {
  ...
  if (isPending) {
    return <div dir="rtl" className="min-h-screen bg-background p-8"><AdminLoading label="בודק הרשאות…" /></div>;
  }

  if (error) {
    return <div dir="rtl" className="min-h-screen bg-background p-8"><AdminError error={error} /></div>;
  }
      ...
      <div dir="rtl" className="grid min-h-screen place-items-center bg-background p-6">
        <div className="w-full max-w-md rounded-3xl border border-border bg-card p-8 text-center shadow-sm">
          ...
          {me.needsBootstrap ? (
            ...
            <p className="mt-2 text-sm text-muted-foreground">אין לך הרשאות ניהול. פנה למנהל המערכת.</p>
          )}
          <Link to="/" className="mt-4 inline-flex items-center gap-1 text-xs font-bold text-primary">
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-muted/30">
      <header className="sticky top-0 z-30 flex items-center justify-between gap-3 border-b border-border bg-background/90 px-4 py-3 backdrop-blur lg:hidden">
        <button onClick={() => setOpen((v) => !v)} className="grid h-9 w-9 place-items-center rounded-lg border border-border">
          {open ? <X className="h-4 w-4" /> : <Menu className="h-4 w-4" />}
        </button>
        <NitziLogo />
        <Link to="/" className="text-xs font-bold text-primary">לאתר</Link>
      </header>
      ...
      <div className="mx-auto flex w-full max-w-[1600px] gap-6 px-3 py-4 sm:px-6">
        <aside
          className={cn(
            ...
            open ? "fixed inset-x-3 top-16 block rounded-2xl border border-border bg-card p-3 shadow-xl" : "hidden",
            "lg:sticky lg:top-6 lg:h-[calc(100vh-3rem)] lg:rounded-2xl lg:border lg:border-border lg:bg-card lg:p-4",
          )}
        >
          <div className="mb-4 hidden items-center justify-between lg:flex">
            <NitziLogo />
            <Link to="/" className="text-[11px] font-bold text-primary">לאתר</Link>
          </div>
