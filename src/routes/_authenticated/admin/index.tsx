...
function MiniBars({ data, empty }: { data: NamedCount[]; empty: string }) {
  if (data.length === 0)
    return <AdminEmpty title={empty} hint="הנתונים יופיעו כאן ברגע שתהיה פעילות אמיתית." />;
  return (
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <StatCard
          label="סה״כ הזמנות"
          value={o.totalOrders.toLocaleString("he-IL")}
          icon={ShoppingBag}
        />
        ...
        <StatCard
          ...
          hint="הזמנות מתוך חיפושים ב-30 יום"
          icon={Percent}
        />
        <StatCard
          label="משתמשים פעילים"
          value={o.activeUsers.toLocaleString("he-IL")}
          hint="פעילות ב-30 הימים האחרונים"
          icon={Users}
        />
        <StatCard
          label="משתמשים חדשים היום"
          value={o.newUsersToday.toLocaleString("he-IL")}
          icon={UserPlus}
        />
      </div>
      ...
      <SectionCard
        ...
        action={
          <Link to="/admin/users" className="text-xs font-bold text-primary">
            לכל המשתמשים
          </Link>
        }
      >
