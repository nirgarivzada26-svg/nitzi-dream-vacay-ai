...
function fmtDate(iso: string) { return new Date(iso).toLocaleDateString("he-IL", { weekday: "short", day: "2-digit", month: "short" }); }
function fmtDur(min: number) { const h = Math.floor(min / 60), m = min % 60; return `${h}ש׳ ${m ? `${m}ד׳` : ""}`.trim(); }
        ...
        <section className="rounded-3xl border border-border/60 bg-card p-6 shadow-soft">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <h1 className="mt-3 text-3xl font-black text-foreground sm:text-4xl">{flight.airline}</h1>
              <p className="mt-1 text-sm text-muted-foreground">טיסה {flight.flightNumber} · {fmtDate(flight.departAt)}</p>
            </div>
          ...
          <div className="mt-6 grid grid-cols-[1fr_auto_1fr] items-center gap-4 rounded-2xl bg-muted/40 p-5">
            ...
            <div className="flex flex-col items-center">
              ...
              <div className="mt-1 text-[11px] font-bold text-muted-foreground">{fmtDur(flight.durationMinutes)}</div>
              <div className="text-[10px] text-muted-foreground">{flight.stops === 0 ? "ישירה" : `${flight.stops} עצירות`}</div>
            </div>
            <div className="text-center">
