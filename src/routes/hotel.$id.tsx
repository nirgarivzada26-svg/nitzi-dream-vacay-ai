...
export const Route = createFileRoute("/hotel/$id")({
  head: ({ params }) => ({
    meta: [
      ...
      {
        name: "description",
        content: `פרטי מלון מלאים, שירותים, מחיר מעודכן והזמנה. NITZI ${decodeURIComponent(params.id)}`,
      },
...
function HotelDetailPage() {
  ...
  const reasons = [
    ...
    fit === "within"
      ? "המחיר בתוך התקציב שלך."
      : fit === "slightly-over"
        ? "מעט מעל התקציב, אבל שווה את התוספת."
        : "מעל התקציב — נשמר להשוואה בלבד.",
    hotel.distanceToBeachKm != null
      ? `${hotel.distanceToBeachKm} ק״מ מהחוף — מיקום מעולה לחופשת ים.`
      : `במיקום מרכזי ב-${hotel.location}.`,
    ...
    <div dir="rtl" className="min-h-screen bg-background pb-32">
      <header className="sticky top-0 z-30 border-b border-border/60 bg-background/85 backdrop-blur-lg">
        <div className="mx-auto flex w-full max-w-[1600px] items-center justify-between px-4 py-3 sm:px-6">
          ...
          <NitziLogo />
          <button
            onClick={() => toggleCompare({ id: hotel.id, kind: "hotel" })}
            className={`grid h-11 w-11 place-items-center rounded-full border ${inCompare ? "border-primary bg-primary text-white" : "border-border bg-card"}`}
            aria-label="השווה"
          >
          ...
          <div className="space-y-5">
            <section className="relative overflow-hidden rounded-[2rem] shadow-glow">
              ...
              <div className="absolute bottom-0 inset-x-0 p-5 text-white sm:p-6">
                ...
                <h1 className="text-3xl font-black leading-tight drop-shadow-md sm:text-5xl">
                  {hotel.name}
                </h1>
            ...
            <section className="rounded-3xl border border-emerald-200 bg-emerald-50/70 p-4 shadow-soft">
              <div className="flex flex-wrap items-center gap-3 text-sm text-emerald-900">
                ...
                <span className="ms-auto">
                  <WhyNitziButton reasons={reasons} score={hotel.score} />
                </span>
              </div>
            </section>
            ...
            <Section title="על המלון" icon={<HotelIcon className="h-4 w-4" />}>
              <p className="text-[15px] leading-relaxed text-foreground">
                {explainHotel(hotel, cache.answers)} מלון של {hotel.stars} כוכבים במיקום{" "}
                {hotel.location}, עם דירוג אורחים גבוה של {hotel.guestRating.toFixed(1)} מתוך 10 על
                סמך {hotel.reviewsCount.toLocaleString()} ביקורות אמיתיות.
              </p>
            </Section>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Section title="מה כלול" icon={<CheckCircle2 className="h-4 w-4" />}>
                <ul className="space-y-2 text-sm">
                  <li className="flex gap-2">
                    <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-600" /> {nights}{" "}
                    לילות ב{hotel.name}
                  </li>
                  {hotel.amenities.includes("breakfast") && (
                    <li className="flex gap-2">
                      <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-600" /> ארוחת
                      בוקר יומית
                    </li>
                  )}
                  {hotel.amenities.includes("wifi") && (
                    <li className="flex gap-2">
                      <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-600" /> Wi-Fi
                      חינם
                    </li>
                  )}
                  <li className="flex gap-2">
                    <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-600" /> תמיכה 24/7
                    של NITZI
                  </li>
                </ul>
              </Section>
              <Section title="מה לא כלול" icon={<XCircle className="h-4 w-4" />}>
                <ul className="space-y-2 text-sm">
                  <li className="flex gap-2">
                    <XCircle className="mt-0.5 h-4 w-4 shrink-0 text-rose-500" /> טיסות (רכיב נפרד)
                  </li>
                  <li className="flex gap-2">
                    <XCircle className="mt-0.5 h-4 w-4 shrink-0 text-rose-500" /> העברות משדה תעופה
                  </li>
                  <li className="flex gap-2">
                    <XCircle className="mt-0.5 h-4 w-4 shrink-0 text-rose-500" /> ביטוח נסיעות
                  </li>
                </ul>
              </Section>
            </div>
            ...
            <Section title="מיקום" icon={<MapPin className="h-4 w-4" />}>
              <div className="relative h-48 overflow-hidden rounded-2xl border border-border">
                ...
                <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 text-center">
                  ...
                  <div className="mt-1 text-xs font-black text-white drop-shadow">
                    {hotel.location}
                  </div>
                </div>
              </div>
              <div className="mt-3 flex flex-wrap gap-3 text-xs text-muted-foreground">
                {hotel.distanceToCenterKm != null && (
                  <span>
                    ...
                    {hotel.distanceToCenterKm} ק״מ ממרכז העיר
                  </span>
                )}
                {hotel.distanceToBeachKm != null && (
                  <span>
                    <Waves className="mr-0.5 inline h-3 w-3" />
                    {hotel.distanceToBeachKm} ק״מ מהחוף
                  </span>
                )}
              </div>
            </Section>
            ...
            <Section title="תנאי ביטול" icon={<Shield className="h-4 w-4" />}>
              <p className="text-sm text-foreground">
                ביטול חינם עד 7 ימים לפני צ'ק-אין. לאחר מכן — חיוב של לילה ראשון.
              </p>
            </Section>

            <SimilarPicks
              catalog={catalog}
              excludeSlug={dest.slug}
              title="מלונות דומים שאולי תאהב"
            />
          ...
          <aside className="hidden lg:block">
            <div className="sticky top-24 space-y-3 rounded-3xl border border-border bg-card p-5 shadow-soft">
              <div className="flex items-baseline justify-between">
                ...
                <span
                  className={`rounded-full px-2.5 py-1 text-[11px] font-bold ${fit === "within" ? "bg-emerald-100 text-emerald-800" : fit === "slightly-over" ? "bg-amber-100 text-amber-900" : "bg-rose-100 text-rose-800"}`}
                >
