...
export const Route = createFileRoute("/compare")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "השוואה בין מלונות או חבילות שבחרת: מחיר, דירוג, שירותים ותנאי ביטול.",
      },
        ...
        {empty ? (
          <div className="mt-8 rounded-3xl border border-dashed border-border bg-card p-10 text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              חזור לעמוד התוצאות והוסף מלונות או חבילות באמצעות כפתור "השווה".
            </p>
            <button
              onClick={() => navigate({ to: "/result" })}
      ...
      <table className="w-full min-w-[720px] border-separate border-spacing-2">
            ...
            {hotels.map((h) => (
              <th key={h.id} className="rounded-2xl bg-card p-3 text-right align-top">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <button
                    onClick={() => toggleCompare({ id: h.id, kind: "hotel" })}
        ...
        <tbody className="text-sm">
          <Row
            label="מחיר ללילה"
            cells={hotels.map((h) => (
              <span className="font-black text-foreground">{fmtILS(h.pricePerNight)}</span>
            ))}
          />
          <Row
            label={`סה״כ ל-${nights} לילות`}
            cells={hotels.map((h) => (
              <span className="font-black text-primary">{fmtILS(h.pricePerNight * nights)}</span>
            ))}
          />
          <Row
            label="כוכבים"
          ...
          <Row
            label="דירוג אורחים"
            cells={hotels.map((h) => (
              <span className="font-bold">{h.guestRating.toFixed(1)}/10</span>
            ))}
          />
          <Row
            label="מרחק מהחוף"
            cells={hotels.map((h) =>
              h.distanceToBeachKm != null ? `${h.distanceToBeachKm} ק״מ` : "—",
            )}
          />
          <Row
            label="מרחק מהמרכז"
            cells={hotels.map((h) =>
              h.distanceToCenterKm != null ? `${h.distanceToCenterKm} ק״מ` : "—",
            )}
          />
          <Row
            label="ציון NITZI"
            cells={hotels.map((h) => (
              <span className="rounded-full bg-gradient-sunset px-2.5 py-0.5 text-xs font-black text-white">
                {h.score}%
              </span>
            ))}
          />
          {["breakfast", "pool", "spa", "wifi", "beachfront", "family"].map((am) => (
            <Row
              key={am}
              label={amenityLabel(am)}
              cells={hotels.map((h) =>
                h.amenities.includes(am) ? (
...
function PackageCompareTable({ ids }: { ids: string[] }) {
  const pkgs = ids.map(findPackage).filter(Boolean) as NonNullable<
    ReturnType<typeof findPackage>
  >[];
  return (
    <div className="mt-6 overflow-x-auto">
      <table className="w-full min-w-[720px] border-separate border-spacing-2">
            ...
            {pkgs.map((p) => (
              <th key={p.id} className="rounded-2xl bg-card p-3 text-right align-top">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <button
                    onClick={() => toggleCompare({ id: p.id, kind: "package" })}
                    className="grid h-7 w-7 place-items-center rounded-full border border-border bg-background"
                  >
                    <X className="h-3 w-3" />
                  </button>
        ...
        <tbody className="text-sm">
          <Row
            label="מחיר כולל"
            cells={pkgs.map((p) => (
              <span className="font-black text-primary">{fmtILS(p.totalPrice)}</span>
            ))}
          />
          <Row
            label="במקום"
            cells={pkgs.map((p) => (
              <span className="text-muted-foreground line-through">{fmtILS(p.separatePrice)}</span>
            ))}
          />
          <Row
            label="חיסכון"
            cells={pkgs.map((p) => (
              <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-bold text-emerald-800">
                {Math.round((p.savings / p.separatePrice) * 100)}%
              </span>
            ))}
          />
          ...
          <Row
            ...
            cells={pkgs.map(
              (p) =>
                `${p.flight.airline} · ${p.flight.stops === 0 ? "ישירה" : `${p.flight.stops} עצירות`}`,
            )}
          />
          <Row
            label="ציון NITZI"
...
function Row({ label, cells }: { label: string; cells: React.ReactNode[] }) {
  return (
    <tr>
      ...
      {cells.map((c, i) => (
        <td key={i} className="rounded-2xl bg-card p-3 text-right">
          {c}
        </td>
      ))}
    </tr>
