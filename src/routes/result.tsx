...
export const Route = createFileRoute("/result")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "הצעת חופשה בהתאמה אישית מ-NITZI: יעד, מסלול, מלונות, מזג אוויר ותקציב.",
      },
    ...
    return (
      ...
      >
        <div className="max-w-md">
          ...
          <Link
            to="/"
            className="mt-5 inline-block rounded-2xl bg-gradient-sunset px-5 py-2.5 text-sm font-black text-white shadow-glow"
          >
            חזרה לחיפוש
          </Link>
        </div>
...
function ResultView({ answers, dest }: { answers: QuizAnswers; dest: Destination }) {
  ...
  const cheapestFlight = useMemo(
    () => flights.slice().sort((a, b) => a.price - b.price)[0],
    [flights],
  );
  const fastestFlight = useMemo(
    () => flights.slice().sort((a, b) => a.durationMinutes - b.durationMinutes)[0],
    [flights],
  );
  ...
  return (
    <div dir="rtl" className="relative min-h-screen bg-background pb-24">
      <div
        aria-hidden
        className="pointer-events-none absolute -top-32 -right-24 h-80 w-80 rounded-full bg-gradient-sunset opacity-30 blur-3xl"
      />
      ...
      <div className="mx-auto w-full max-w-[1600px] space-y-5 pt-5">
        ...
        <div className="mx-5 grid grid-cols-3 gap-2">
          ...
          <MiniStat
            ...
            label="טיסה מישראל"
            value={`${dest.flightHours} שעות`}
          />
          <MiniStat
            icon={<Calendar className="h-4 w-4" />}
            label="משך"
            value={`${answers.days} ימים`}
          />
        </div>
        ...
        <div className="mx-5 grid gap-5 lg:grid-cols-2 [&>section]:mx-0">
          ...
          <Card>
            ...
            <ul className="mt-3 space-y-2 text-sm text-foreground">
              <Li>
                בחרת {typeLabel} — {dest.name} מספק/ת בדיוק את זה.
              </Li>
          ...
          <Card>
            ...
            <div
              className={`mt-3 rounded-2xl p-4 text-sm font-semibold ${enoughBudget ? "bg-emerald-50 text-emerald-900" : "bg-amber-50 text-amber-900"}`}
            >
              {enoughBudget
          ...
          <Card>
            ...
            <ol className="mt-4 space-y-3">
              {dest.itinerary.slice(0, answers.days).map((d, i) => (
                <li
                  key={i}
                  className="relative flex gap-3 rounded-2xl border border-border/70 bg-muted/40 p-3"
                >
          ...
          <Card>
            ...
            <div className="relative mt-3 h-40 overflow-hidden rounded-2xl border border-border">
              ...
              <div
                ...
                style={{
                  backgroundImage:
                    "radial-gradient(circle at 20% 30%, rgba(255,255,255,.35) 0 2px, transparent 3px), radial-gradient(circle at 70% 60%, rgba(255,255,255,.25) 0 2px, transparent 3px), radial-gradient(circle at 40% 80%, rgba(255,255,255,.2) 0 2px, transparent 3px)",
                }}
              />
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[11px] text-muted-foreground">
                  נבחרו עבורך לפי התקציב, הסגנון וההעדפות
                </p>
              </div>
            </div>
          </div>
          ...
          {hotels.length === 0 || flights.length === 0 ? (
            ...
            <div className="mt-4 space-y-3">
              ...
              {hotels.slice(0, 2).map((h, i) => (
                <PickWrap
                  key={h.id}
                  kind={i === 0 ? "המלון המושלם" : "אלטרנטיבה מצוינת"}
                  reason={i === 0 ? "הציון הגבוה ביותר בהתאמה אישית" : "איזון טוב בין מחיר לאיכות"}
                >
        ...
        <Card>
          ...
          <div className="mt-3 space-y-2 text-sm">
            <Row
              label="טיסות"
              value={`₪${(answers.budget * 0.35 * answers.people).toLocaleString(undefined, { maximumFractionDigits: 0 })}`}
            />
            <Row
              label={`מלון · ${answers.days} לילות`}
              value={`₪${(answers.budget * 0.35 * answers.people).toLocaleString(undefined, { maximumFractionDigits: 0 })}`}
            />
            <Row
              label="אוכל ובילויים"
              value={`₪${(answers.budget * 0.2 * answers.people).toLocaleString(undefined, { maximumFractionDigits: 0 })}`}
            />
            <Row
              label="אטרקציות ותחבורה"
              value={`₪${(answers.budget * 0.1 * answers.people).toLocaleString(undefined, { maximumFractionDigits: 0 })}`}
            />
        ...
        <div className="mx-5 flex flex-col gap-2 pt-2">
          <Link
            to="/ai"
            className="flex items-center justify-center gap-2 rounded-2xl bg-gradient-sunset py-3 text-center text-sm font-black text-white shadow-glow"
          >
          ...
          <div className="grid grid-cols-2 gap-2">
            <button
              onClick={() => setSaved((s) => !s)}
              className="flex items-center justify-center gap-2 rounded-2xl border border-border bg-card py-3 text-sm font-bold text-foreground"
            >
              <Bookmark className={`h-4 w-4 ${saved ? "fill-primary text-primary" : ""}`} />{" "}
              {saved ? "נשמר" : "שמור חופשה"}
            </button>
            <button
              onClick={share}
              className="flex items-center justify-center gap-2 rounded-2xl border border-border bg-card py-3 text-sm font-bold text-foreground"
      ...
      <div className="w-full max-w-sm text-center">
        ...
        <ul className="mt-6 space-y-2 text-right text-sm">
          {lines.map((l, idx) => (
            ...
            >
              <span
                className={`h-2 w-2 rounded-full ${idx <= i ? "bg-primary animate-pulse" : "bg-muted-foreground/50"}`}
              />
...
function MiniStat({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="rounded-2xl border border-border/70 bg-card p-3 text-center shadow-soft">
      <div className="mx-auto grid h-8 w-8 place-items-center rounded-lg bg-gradient-ocean text-white">
        {icon}
      </div>
...
function SkeletonRow() {
  return <div className="h-20 animate-pulse rounded-2xl border border-border/70 bg-muted/40" />;
}
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              ...
              <div className="mt-0.5 flex items-center gap-2 text-xs text-muted-foreground">
                <Stars n={hotel.stars} />
                <span>
                  · {hotel.guestRating.toFixed(1)}/10 ({hotel.reviewsCount.toLocaleString()})
                </span>
              </div>
          ...
          <div className="mt-2 flex flex-wrap gap-1">
            {hotel.amenities.slice(0, 5).map((a) => (
              <span
                key={a}
                className="rounded-full bg-card px-2 py-0.5 text-[10px] font-semibold text-foreground"
              >
                {amenityLabel(a)}
              </span>
            ))}
    ...
    >
      ...
      <div className="mt-2 flex flex-wrap items-center justify-between gap-2">
        <span className="text-[11px] text-muted-foreground">
          <Plane className="mr-0.5 inline h-3 w-3" />
          {pkg.flight.airline} · {pkg.flight.stops === 0 ? "ישירה" : `${pkg.flight.stops} עצירות`}
        </span>
        <div className="flex items-center gap-1.5">
