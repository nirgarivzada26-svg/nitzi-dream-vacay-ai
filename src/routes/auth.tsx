...
export const Route = createFileRoute("/auth")({
  ...
  head: () => ({
    meta: [
      { title: "התחברות — NITZI" },
      { name: "description", content: "התחבר או הירשם ל-NITZI כדי לשמור דילים, לצפות בהזמנות ולקבל המלצות בהתאמה אישית." },
...
function AuthPage() {
  ...
  const applyOptIn = async () => {
    ...
    try { await updateNotifPrefs({ deals: true, email: true }); } catch {}
  };
  ...
  const handleEmail = async (e: React.FormEvent) => {
    ...
    const returnUrl = redirect ? `${window.location.origin}/auth?redirect=${encodeURIComponent(redirect)}` : window.location.href;
  ...
  return (
    <div dir="rtl" className="relative min-h-screen bg-gradient-to-b from-sand/60 via-background to-background pb-16">
      ...
      <div className="mx-auto mt-10 grid w-full max-w-6xl gap-10 px-5 sm:px-8 lg:mt-16 lg:grid-cols-2">
        ...
        <div className="hidden flex-col justify-center lg:flex">
          ...
          <h1 className="mt-4 text-5xl font-black leading-[1.05] text-foreground">
            כל החופשות שלך <br /><span className="text-gradient-sunset">במקום אחד</span>
          </h1>
        ...
        <div className="mx-auto w-full max-w-md rounded-[2rem] border border-border bg-card/95 p-6 shadow-glow backdrop-blur sm:p-8">
          <div className="text-center">
            ...
            <p className="mt-1 text-sm text-muted-foreground">התחבר במייל או עם Google — לוקח שניות.</p>
          </div>
          ...
          {status === "sent" ? (
            <div className="mt-6 rounded-2xl border border-emerald-200 bg-emerald-50 p-5 text-center">
              ...
              <p className="mt-1 text-[12px] text-emerald-800">
                לחץ על הקישור שקיבלת בכתובת <span className="font-black">{email}</span>. תוחזר אוטומטית לאותו מקום.
              </p>
            </div>
          ) : (
            <>
              ...
              <form onSubmit={handleEmail} className="space-y-2">
                ...
                {error && <div className="rounded-xl bg-rose-50 p-2 text-[11px] font-bold text-rose-800">{error}</div>}
                ...
                >
                  {status === "loading" ? <Loader2 className="h-4 w-4 animate-spin" /> : <Mail className="h-4 w-4" />} שלח קישור התחברות
                </button>
              </form>
            </>
          )}
