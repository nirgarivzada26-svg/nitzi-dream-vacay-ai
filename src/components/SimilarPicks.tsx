...
export function SimilarPicks({ catalog, excludeSlug, title = "אולי תאהב גם..." }: Props) {
  const picks = catalog
    .filter((d) => d.slug !== excludeSlug && d.hasOffers)
    .slice(0, 4);
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
        {picks.map((d) => (
          <Link
            ...
            to="/result"
            onClick={() => {
              try { sessionStorage.setItem("nitzi:answers", JSON.stringify({ type: null, destination: d.slug, days: 5, budget: d.avgBudgetPerPerson, people: 2, style: null })); } catch {}
            }}
