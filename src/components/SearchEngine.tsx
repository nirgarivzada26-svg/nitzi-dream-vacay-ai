...
export function SearchEngine({ size = "md" }: { size?: "md" | "lg" }) {
  ...
  return (
    <>
      <div
        className={`rounded-[2rem] border border-white/50 bg-white/90 shadow-glow backdrop-blur-2xl ${pad}`}
      >
        <div className={`grid gap-2 ${gridCols}`}>
          <Field lg={lg} icon={<PlaneTakeoff className="h-4 w-4" />} label="מאיפה">
            <input
              ...
              className={`w-full bg-transparent ${inputTxt} font-bold text-foreground outline-none`}
            />
          </Field>
          <button
            type="button"
          ...
          >
            ...
            <div
              className={`mt-0.5 flex items-center justify-between gap-1 ${inputTxt} font-bold ${destination ? "text-foreground" : "text-muted-foreground"}`}
            >
        ...
        <div
          className={`mt-3 rounded-2xl border border-border/60 bg-muted/50 ${lg ? "p-4" : "p-3"}`}
        >
          <div className="flex items-center justify-between text-xs font-bold text-muted-foreground">
            <span className="flex items-center gap-1.5">
              <Wallet className="h-3.5 w-3.5" /> תקציב לאדם
            </span>
            <span className={`text-gradient-sunset ${lg ? "text-base" : "text-sm"}`}>
...
function Field({
  ...
  return (
    <label
      className={`block rounded-2xl border border-border/60 bg-white/70 transition focus-within:border-primary focus-within:ring-4 focus-within:ring-primary/15 ${lg ? "px-4 py-3" : "px-3 py-2"}`}
    >
      <div className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
