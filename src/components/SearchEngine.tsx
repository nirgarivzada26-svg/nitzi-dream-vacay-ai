  ...
  return (
    <div className={`rounded-[2rem] border border-white/50 bg-white/90 shadow-glow backdrop-blur-2xl ${pad}`}>
      <div className={`grid gap-2 ${gridCols}`}>
        ...
        <Field lg={lg} icon={<MapPin className="h-4 w-4" />} label="לאן">
          <input
            value={destination}
            onChange={(e) => setDestination(e.target.value)}
            placeholder="בחר יעד..."
            className={`w-full bg-transparent ${inputTxt} font-bold text-foreground outline-none placeholder:font-medium placeholder:text-muted-foreground`}
          />
        </Field>
