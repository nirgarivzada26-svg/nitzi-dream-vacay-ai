...
export function TripTimeline({
  ...
  const steps: { icon: React.ReactNode; title: string; desc: string; tone: string }[] = [
    {
      ...
      title: "יציאה מישראל",
      desc: `המראה מנתב״ג ל${destinationName}`,
      tone: "from-sky-500 to-blue-600",
    },
    {
      icon: <PlaneLanding className="h-5 w-5" />,
      title: "נחיתה",
      desc: `הגעה ל${destinationName} — העברות ליעד`,
      tone: "from-emerald-500 to-teal-600",
    },
    {
      icon: <Hotel className="h-5 w-5" />,
      title: "צ'ק-אין למלון",
      desc: "מנוחה קצרה ופתיחה של החופשה",
      ...
      : []),
    {
      ...
      title: "טיסת חזור",
      desc: `חזרה ל${originCity} — סוף החופשה`,
  ...
  return (
    <div className="relative">
      <div
        className="absolute right-[22px] top-0 h-full w-0.5 bg-gradient-to-b from-primary/40 via-border to-primary/40"
        aria-hidden
      />
      <ol className="space-y-4">
        {steps.map((s, i) => (
          <li key={i} className="relative flex gap-4">
            <div
              className={`grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-gradient-to-br ${s.tone} text-white shadow-glow`}
            >
              {s.icon}
            </div>
